...
public static class PortalNativeSalesEndpoints
{
    ...
    private static Task<IResult> PostSaleAsync(
        ...
        PostAsync(http, body, db, NativeDocumentProcessor.SalesOrder, "portal-sale", "customerCode", "Satış", linesRequired: true, ct);

    private static Task<IResult> PostPurchaseAsync(
        HttpContext http, [FromBody] PortalNativeDocumentRequest? body, [FromServices] CentralApiDbContext db, CancellationToken ct) =>
        PostAsync(http, body, db, NativeDocumentProcessor.PurchaseReceipt, "portal-purchase", "supplierCode", "Alış", linesRequired: false, ct);

    private static Task<IResult> PostReturnAsync(
        HttpContext http, [FromBody] PortalNativeDocumentRequest? body, [FromServices] CentralApiDbContext db, CancellationToken ct) =>
        PostAsync(http, body, db, NativeDocumentProcessor.SalesReturn, "portal-return", "customerCode", "İade", linesRequired: true, ct);
    ...
    private static async Task<IResult> PostAsync(
        ...
        string partyField, string kindLabel, bool linesRequired, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(party)) return Invalid($"{partyField} is required.");
        if (linesRequired && body.Lines.Count == 0) return Invalid("At least one line is required.");
        ...
        var exists = await db.MobileRecords.AsNoTracking()
            .AnyAsync(r => r.TenantId == tenant!.Id && r.Entity == "customers" && r.RecordKey == party && !r.IsDeleted, ct);
        if (!exists) return Invalid($"No customer or supplier with code '{party}'.");
        ...
        var audit = new PortalNativeWriteHelpers.AuditInfo(
            ...
            Summary: $"{kindLabel}: {total.ToString("0.00", Turkish)} TL — {party}", BeforeJson: null);
        return await PortalNativeWriteHelpers.BookNativeDocumentAsync(
            http, db, tenant!, user!, documentType,
            PortalNativeWriteHelpers.OperationKey(keyPrefix, party, body.OperationId), payload, "SALES_DOCUMENT_REJECTED", ct, audit);
    }

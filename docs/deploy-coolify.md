...
4. **Generate a password** (Coolify stores it; copy it — you'll paste it
   into the central API's environment as `POSTGRES_PASSWORD`).
5. Deploy. Note the internal service hostname (something like
   `erpbridge-pg-xxxx` — Coolify shows it on the database's resource
   detail page).
...
passwords — never commit them, never paste them into logs, never email
them in cleartext.
...
2. **Git Repository** — point at your ErpBridge fork. Branch: `main` (or
   your release branch).
3. **Docker Compose Location** — leave at `docker-compose.coolify.yml`.
4. **Base Directory** — leave empty (the file is at the repo root).
5. **Build Pack** — leave "Dockerfile" / "Automatic" detection on.
...
7. On the `centralapi` service detail page:
   ...
   - **Environment Variables**:
     - `POSTGRES_PASSWORD` → *(Secret)* — paste the password from step 2.
     - `JWT_SIGNING_KEY` → *(Secret)* — paste from step 3.
     - `ADMIN_SEED_EMAIL` → `ops@erpbridge.example.com`.
     - `ADMIN_SEED_PASSWORD` → *(Secret)* — paste from step 3.
   - **Healthchecks** — leave Coolify's default HTTP check on `/health`.
...
2. Go to **Tenants → New Tenant**. Create your first customer tenant.
3. Go to **Licenses → Issue license**. Paste the generated key into the
...
5. To start receiving customer jobs over the API:
   - **API Keys → Create key** for the tenant.
...
snapshot alone — the customer holds the raw values).
...
| Admin panel shows "Network error" on every action | `CENTRALAPI_BASE_URL` is set to the internal Docker hostname from inside the admin container — that's wrong. | Use the **public** URL `https://api.erpbridge.example.com`. |
| `401 Unauthorized` everywhere | Clock drift between Coolify's server and your laptop. JWT validation rejects ±30s skew; bigger drift 401s every call. | Sync clocks via `chrony` or `systemd-timesyncd`. |
...
|---|---|---|---|
| `ConnectionStrings__CentralApi` | yes | `Host=erpbridge-pg;Port=5432;Database=erpbridge;Username=erpbridge;Password=…` | Use the **internal** Coolify hostname. |
| `Jwt__SigningKey` | yes | (64 hex chars) | ≥32 bytes. HS256 requires it. |
...
| `ASPNETCORE_URLS` | no | `http://+:8080` | Set by Dockerfile; only override for debugging. |

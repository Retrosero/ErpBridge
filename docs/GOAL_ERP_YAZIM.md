...
| ID | Görev | Kabul ölçütü |
|---|---|---|
| Y2a | `Erp.Abstractions` komutları — üçü de ortak **`ErpDocumentHeader`** taşır (`ExternalId, OccurredAt, CustomerCode, SalespersonCode?, ErpUserNo, Series, Description?, ExpectedTotal`; idempotency, tarih, kullanıcı ve cari bu başlıktan — PR #74 Codex bulgusu): **`SalesDocumentCommand`** (`Header`, `Kind: Order/Dispatch/Invoice`, `WarehouseNo, PriceListNo, ApprovalMode`, `Settlement: Open/Cash/Card/Transfer` + `SettlementAccountCode?`, `Lines[StockCode, Quantity, UnitPointer, ListUnitPrice, LineDiscountPct, CustomerDiscountPct, GeneralDiscountPct, Note?]`, `ExtraPayments?`); **`SalesReturnCommand`** (`Header`, `WarehouseNo, PriceListNo`, `Settlement: Open/Cash/Bank` + hesap kodu, `Lines[StockCode, Quantity, UnitPointer, ListUnitPrice, ConditionPct, Reason]`); **`CollectionCommand`** (`Header`, `Payments[Method: Cash/Card/Transfer/Cheque/Note, Amount, DueDate, AccountCode, Installments?, SurchargeAmount?, Cheque?{No, BankName, Branch, AccountNo, Drawer}, Note?{No, Debtor}]`). `IErpAdapter.WriteSalesDocumentAsync / WriteSalesReturnAsync / WriteCollectionDocumentAsync` (Logo iskeleti `NotSupported` sonuç) | Derleme; `SecondAdapterSeamTests` yeşil |
...
| Y4g | **KDV oranı ve telefon toplamı Mikro ile aynı** (2026-09-17 bulgusu): sunucu telefona ürün KDV oranı göndermiyor, telefon her ERP ürününe `kdv ?: 20` uyguluyor (bu firmada Mikro pointer 1 = %0) ve KDV'yi genel iskontodan önceki tutardan hesaplıyor (`SalesScreen.kt` `taxTotal`). Ajan okuyucusu stok KDV oranını `fn_VergiYuzde(sto_toptan_vergi)` ile okur (`SnapshotProjectionVersion` artar), sunucu `urunler`'e `kdv` olarak koyar, telefon bu oranı ve iskontolu net üzerinden KDV'yi kullanır; fiyat listesi KDV dahilse (`sfl_kdvdahil`, bölüm `fiyatTanim`) telefon KDV eklemez | Ajan canlı okuma testi (DEMO: oran = `fn_VergiYuzde`); sunucu montaj testi; telefon birim testi: aynı sepet için telefon toplamı = Y3b hesaplayıcısının toplamı (Mikro kuralları) |
...
- Y0e sonrası müşteri PC'sine yeni ajan sürümü + cari hareketleri bölümünün sıfırdan yüklenmesi ("Sıfırdan Kur"); yoksa
  sunucudaki eski satırlar yanlış müşteri/tür ile kalır.

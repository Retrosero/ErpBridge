...
public static class TxtCsvImportAktarimParametreleri
{
	public static List<string> GetParametreAdlari(SqlBaglantiBilgileri BaglantiBilgileri, string DBName)
	{
		...
		string commandText = "SELECT ParametreAltGrubu FROM _FORA_PARAMETRELER WITH (NOLOCK) WHERE ParametreProgram='GenelAktarim' AND ParametreAnaGrubu='TxtCsvAktarimSablon' GROUP BY ParametreAltGrubu ORDER BY ParametreAltGrubu";
		try
		{
	...
	{
		string commandText = "DELETE _FORA_PARAMETRELER WHERE ParametreProgram='GenelAktarim' AND ParametreAnaGrubu='TxtCsvAktarimSablon' AND ParametreAltGrubu=@ParametreAltGrubu";
		try
		{
			SqlDB sqlDB = new SqlDB();
			sqlDB.ConnectionOpen(BaglantiBilgileri, DBName);

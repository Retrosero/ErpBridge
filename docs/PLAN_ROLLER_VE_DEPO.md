...
~~Telefon uygulamasının depo ekranını sunucuya bağlama~~ (panel goal P5 ile yapıldı) · barkodla toplama doğrulaması · araç/rota sevkiyat planlama ·
...
| 6 — Depo personeli sayfası | ✅ | [#63](https://github.com/Retrosero/ErpBridge/pull/63), [#65](https://github.com/Retrosero/ErpBridge/pull/65), [#66](https://github.com/Retrosero/ErpBridge/pull/66), [siparis_cepte#67](https://github.com/Retrosero/siparis_cepte/pull/67) | [GOAL_PANEL_GELISTIRMELERI.md](GOAL_PANEL_GELISTIRMELERI.md): panel `/depo` (sekmeler, tek dokunuşla adım, toplama listesi, geri al/iptal/yeniden ata, canlı long-poll), modül açılırken geri doldurma (P4); telefon Depo ekranı aynı sunucu kuyruğunda, yalnız depo rollü kullanıcı telefonda yalnız Depo'yu görür, en düşük sürüm 1.5.235 (P5, internal). Cihazda uçtan uca test insan kapısında ([GOAL_PANEL_DURUM.md](GOAL_PANEL_DURUM.md) Seni Bekleyenler) |
| 7 — Canlı Depo Ekranı | ✅ | Faz 49 | `display_devices`, `display_pairing_codes` (migration yalnız yeni tablo); `DisplayEndpoints`: anonim kod + gizli anahtar, yönetici sahiplenir, tek seferlik `scope=display` token, `/display/board` + `/display/events`, iptal; ekran başına hız sınırı. Panel `/ekran` (kiosk: kod → koyu pano, eşik renkleri, sayfa dönüşü, bağlantı kaybında son pano, Wake Lock, kopan devrede yeniden yükleme) ve `/ekranlar` (eşleştir, liste, iptal, depo eşikleri). **Sapmalar:** canlı akış sayfa başına long-poll (V5'teki firma başına ortak abonelik yapılmadı; ekran başına hız sınırıyla yük yönetildi); paketlendi kolonunda yalnız sarı eşik. Tarayıcıda: yönetici arayüzünden eşleştirme, "başla" ile kart kolon değiştirdi, iptal ~4 sn'de koda döndü, yenilemede pano korundu |
| 8 — Yönetici & performans paneli | ✅ | Faz 50 | `FulfillmentMetrics` (saf ölçüm) + `FulfillmentReports`; `/portal/warehouse/dashboard`, `/portal/warehouse/performance` (92 gün), sipariş detayına `times`. Migration yalnız indeks `(TenantId, OccurredAtUtc)`. Panel: Özet'te depo kartları, `/depo-performans` (toplamlar, personel, en uzun bekleyen/hazırlanan, günler), `/depo-performans/siparis/{id}` zaman çizelgesi. **Sapmalar:** sipariş son paketleyene yazılır; "geciken" TV eşikleriyle anlık sayılır; zaman çizelgesi yalnız rapordan açılır (depo sayfası adım 6'da). Bilinen senaryo testleri: geri alınan başlama, geri alınan paketleme, iptal, gece yarısı sınırı |
| 9 — Canlıya alma ve rehber | ⬜ | — | |

Her adım kullanıcının "Adım N'i başlat" onayıyla başlar.
**İstisna (kullanıcı, 2026-09-16): Adım 7 bittiğinde (PR main'e birleşip canlıda doğrulanınca) Adım 8 ayrıca onay beklemeden başlar**; Adım 8'in PR/merge/canlıya alması da bu onayla kapsanır. Adım 9 yine ayrı onay ister.

...
|---|---|---|---|
| P0 — Katalog çıkarımı (ön koşul) | 7 | 6 | 🔄 |
...
| P0d | Tip çıkarsama ve doğrulama | ✅ | — | P0b+P0c ile birlikte tamamlandı. 9 editör tipi: `boolean` 965, `text` 740, `integer` 117, `choice` 70, `decimal` 29, `color` 6, `reference` 4, `composite` 4, `multilineText` 1. `unknown` kalmadı (test bunu sabitliyor). **Referans alanları** Fora'nın kendi seçici kontrollerinden çıkarıldı (`CariSecimi`→cari, `DepoSecimi`→depo, `KargoSecimi`→kargo, `EkipKoduSecimi`→ekipKodu) — D12'nin panelde seçici göstermesi gereken alanları. `composite`, Fora'ya özgü ekranla düzenlenen değer (rapor tanımı JSON'u) |

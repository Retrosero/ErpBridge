...
| P2a | Stok arama ucu (sayfalı, filtreli) | ✅ | #PR | P2a+b+c tek PR. `Portal/PortalStockCatalog` + `GET /portal/stock/search`; önbellek anahtarı stok varlıklarının `MAX(UpdatedSeq)`+sayısı (#54 Codex bulgusu). 3 ilişkisel test (ERP alanları, hatalı sorgu/yetki, ERP'siz kart + önbelleğin yeni kartı ve satışı görmesi). **Ölçüm (SQLite, 20.000 ürün × 4 kayıt):** ilk arama 714 ms, sonraki 86 ms, facets 76 ms |
| P2b | Stok filtre seçenekleri ucu | ✅ | #PR | `GET /portal/stock/facets`: grup/alt grup (üst grubuyla)/marka/reyon sayılarıyla, depo ve fiyat listesi adları, `hasMovementDates`, `hasReserved` |
| P2c | Stok sayfası | ✅ | #PR | Sunucu sayfalı tablo (25/50/100/250; ilk/önceki/sonraki/son), kolon başlığıyla sıralama (sayılarda ilk tık azalan), durum sekmeleri (Tümü/Stokta/Tükenen/Eksi/Eşik altı + eşik), sağdan filtre paneli (depo, fiyat listesi, miktar/fiyat aralığı, hareketsizlik — yalnız ERP; aramalı çoklu seçim grup/alt grup/marka/reyon), etkin filtre çipleri, depo kolonları anahtarı, satır açılınca depo/fiyat/barkod. Durum URL'de (`Api/StockFilter`). 200 sınırı kalktı. bUnit +9. Yerelde Docker PostgreSQL ile 133 ürünlük tohumda filtre/sıralama/URL doğrulandı |

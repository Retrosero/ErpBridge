...
public class EkipKoduSecimi : UserControl
{
	private Ekip _private_selecteditem;

	private string _private_CodeEmptyMessage = "Ekip seçiniz";
	...
	public Ekip SelectedItem
	{
		...
		set
		{
			...
			{
				be_kod.Text = _private_selecteditem.ekp_kodu;
			}
			else
			{
	...
	private void be_kod_ButtonClick(object sender, ButtonPressedEventArgs e)
	{
		...
		{
			F10_Ekip_Secimi_V16 f10_Ekip_Secimi_V = new F10_Ekip_Secimi_V16();
			f10_Ekip_Secimi_V.Setup(_mikrouygulamabilgileri, "EKIP_TANIMLARI_CHOOSE_2", be_kod.Text, AllowMultiSelect: false, IlkAramaAktifOlsun: false);
			if (f10_Ekip_Secimi_V.ShowDialog() == DialogResult.OK)
			{
				SelectedItem = f10_Ekip_Secimi_V._selecteditems[0];
			}
		}
		else
		{
			F10_Ekip_Secimi f10_Ekip_Secimi = new F10_Ekip_Secimi();
			f10_Ekip_Secimi.Setup(_mikrouygulamabilgileri, "EKIP_TANIMLARI_CHOOSE_2", be_kod.Text, AllowMultiSelect: false, IlkAramaAktifOlsun: false);
			if (f10_Ekip_Secimi.ShowDialog() == DialogResult.OK)
			{
				SelectedItem = f10_Ekip_Secimi._selecteditems[0];
			}
		}
	}
	...
	private void be_kod_Validating(object sender, CancelEventArgs e)
	{
		...
		{
			Ekip ekip = EkipData.GetEkip(_mikrouygulamabilgileri.baglantibilgileri, _mikrouygulamabilgileri.MikroFirmaDBName, be_kod.Text);
			if (ekip.ekp_kodu == "")
			{
				e.Cancel = true;
			}
	...
	private void InitializeComponent()
	{
		...
		this.be_kod.Location = new System.Drawing.Point(70, 0);

...
| D2 | Tek tablo: **`log_events`** (kaynak bağımsız). Eski `mobile_telemetry_events` **değiştirilmez/silinmez**; migration son 90 günü bir kez kopyalar, iki eski uç **geçiş süresince iki tabloya** yazar. **Geçiş (L1c):** Admin `/telemetry` ucu `log_events`'ten okur ve iki uç eski tabloya yazmayı bırakır; ancak ondan sonra eski tablo 90 günlük temizlikle boşalır. Düşürülmesi ayrı insan onayı ister | Veri kaybı yok, eski uç sözleşmesi bozulmaz; çift yazım kalıcı olmaz |
...
| D12 | Uzaktan tanılama: Admin "tanılama iste" → `diagnostic_requests`. Ajan bunu **heartbeat yanıt gövdesinden** öğrenir. Telefon **telemetri batch yanıtından** öğrenir; gönderecek olayı olmayan boştaki telefon için mevcut 15 dakikalık `TelemetryWorker` periyodik işi kuyruk boşken `GET /api/v1/mobile/diagnostics/pending` çağırır (en çok 15 dk gecikme). Ajan son log dosyasının maskelenmiş kuyruğunu (en çok 2 MB), telefon yerel halka tamponunu (en çok 500 satır) yükler → `diagnostic_bundles` | Kanal eklemeden mevcut trafiği kullanır |
...
| D15 | Metin arama: PostgreSQL'de `pg_trgm` GIN indeksi ile `ILIKE`. Migration yalnız Npgsql'de ve **tek `DO` bloğunda** önce `CREATE EXTENSION IF NOT EXISTS pg_trgm`, sonra indeksi oluşturur; eklenti reddedilirse `EXCEPTION` dalı yalnız uyarı yazar, migration düşmez (arama indekssiz tarar). SQLite testlerinde `Contains` | Tam metin motoru eklemeden hızlı arama |
...
| L0b | `log_events` tablosu + EF entity + migration: `Id, EventId, Source, TenantId?, OccurredAtUtc, ReceivedAtUtc, Severity, Kind, Operation, Screen, Message(2000), ExceptionType, StackTrace(8000), AppVersion, OsVersion, DeviceModel, DeviceId?, UserId?, AgentId?, SessionId?, CorrelationId?, HttpMethod/Route/Status, DurationMs?, RepeatCount (varsayılan 1), FingerprintId?, PropertiesJson (jsonb, 4000), BreadcrumbsJson`; zamanlar ayrıca Unix ms (`OccurredAtMs`, `ReceivedAtMs` — SQLite `DateTimeOffset` karşılaştıramaz). **Tekillik `(Source, EventId)`**: `TenantId` boş olabildiği için onu içeren unique indeks PostgreSQL'de NULL'lı satırlarda tekrarı engellemezdi; olay kimlikleri GUID. İndeksler: `(OccurredAtMs)`, `(TenantId, OccurredAtMs)`, `(Source, Severity, OccurredAtMs)`, `(CorrelationId)`, `(FingerprintId, OccurredAtMs)`, `(DeviceId, OccurredAtMs)`, `(UserId, OccurredAtMs)`, `(AgentId, OccurredAtMs)`, `(Severity, ReceivedAtMs)`; Npgsql'de `Message` için trigram GIN (D15). Migration son 90 günün `mobile_telemetry_events` satırlarını kopyalar (D2) | Migration smoke testi; kopyalanan satır sayısı testi; eski tablo dokunulmamış |
...
| L1c | **Geçiş (D2):** `GET /admin/telemetry` `log_events`'ten okur (sözleşme aynı), iki telemetri ucu eski tabloya yazmayı bırakır. `/telemetry` sayfası korunur, menüde "Log Merkezi" öne alınır; Dashboard "Mobil hatalar" kartı son 24 saatin ERROR+ sayısını yeni uçtan okur ve `/logs`'a bağlanır | Uç testi: eski tabloya yazım yok, `/admin/telemetry` yeni tablodan döner; bUnit |
...
| L4g | **Yerel log + tanılama (D13):** `AppLog` halka tamponu + "Diğer > Sistem > Tanılama gönder" düğmesi (onay diyaloğu) → `POST /api/v1/mobile/diagnostics`; periyodik `TelemetryWorker` kuyruk boşken `diagnostics/pending` sorar (D12) (sunucu ucu L6c'de; bu görev L6c'ye bağlı) | Birim test: tampon sınırı, maskeleme, boş kuyrukta istek soruluyor |
...
| L6c | **Uzaktan tanılama (D12):** `diagnostic_requests` + `diagnostic_bundles`; Admin ajan/cihaz sayfasında "Tanılama iste"; heartbeat yanıtına `diagnostics: {requestId}` (eski ajan gövdeyi yok sayar — test), telemetri batch yanıtına aynı alan, boştaki telefon için `GET /api/v1/mobile/diagnostics/pending`; `POST /api/v1/agents/diagnostics/{requestId}` ve `POST /api/v1/mobile/diagnostics` (boyut sınırı, maskeleme); ajan tarafı log kuyruğu yükleme; Admin'de paket görüntüleme/indirme | Uçtan uca test; boyut sınırı 413 |
...
- Eski `mobile_telemetry_events` tablosunun düşürülmesi (L1c geçişinden 90 gün sonra, ayrı onay).

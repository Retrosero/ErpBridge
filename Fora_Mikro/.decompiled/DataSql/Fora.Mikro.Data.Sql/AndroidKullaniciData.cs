...
public struct AndroidKullaniciData
{
	public static List<string> GetKullaniciAdlari(SqlBaglantiBilgileri BaglantiBilgileri, string DBName)
	{
		List<string> list = new List<string>();
		string commandText = "SELECT ParametreUser FROM _FORA_PARAMETRELER WITH (NOLOCK) WHERE ParametreProgram='akilli' GROUP BY ParametreUser ORDER BY ParametreUser";
		try
		{
	...
	{
		string commandText = "DELETE _FORA_PARAMETRELER WHERE ParametreProgram='akilli' AND ParametreUser=@ParametreUser";
		try
		{
			SqlDB sqlDB = new SqlDB();
			sqlDB.ConnectionOpen(BaglantiBilgileri, DBName);

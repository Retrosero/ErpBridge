...
#### Takip düzeltmesi: bölüm-set yerine bölüm bazlı önbellek + fatura gruplaması (2026-09-10)

İlk düzeltmenin kod incelemesinde iki artık kalan sorun bulundu:

1. **Aynı büyük bölüm iki kopya önbellekleniyordu.** `StockMovementsAsync` yalnızca `["stockTransactions"]` istiyor, `InvoiceMovementsAsync` ise `["stocks","customerTransactions","stockTransactions"]`. Anahtar **bölüm setine** göre olduğu için, tam da bu bölüm (bu tenant'ta ~90k satır) iki ayrı anahtar altında tam kopya hâlde tutuluyordu — bir senkron koşusu her iki uca da uğrarsa bellek maliyeti neredeyse ikiye katlanıyordu. `AddMemoryCache()`'e `SizeLimit` de verilmemişti, yani TTL dolana kadar büyümeyi durduran hiçbir mekanizma yoktu.

2. **Fatura senkronu hâlâ sayfa başına yeniden gruplanıyordu.** `linesByInvoiceRecNo` (tüm `stockTransactions`'ı fatura no'suna göre gruplayan `GroupBy`+`ToDictionary`) önbelleğin JSON'ı ucuzlatmasına rağmen **her fatura sayfasında** yeniden hesaplanıyordu — DB+parse maliyeti gitmişti ama LINQ gruplama maliyeti (90k satır × sayfa sayısı) kalmıştı.

**Düzeltme:** Önbellek artık **bölüm bazlı** (`android-section:{snapshot.Id}:{section}`) — chunk-birleştirme mantığı `MergeSectionChunksAsync`'e çıkarıldı, `GetAndroidDocumentAsync` istenen bölümleri tek tek önbellekten alıp tek bir `JsonDocument`'te birleştiriyor. `stockTransactions` artık hangi kombinasyon isterse istesin **bir kez** kuruluyor ve paylaşılıyor. `linesByInvoiceRecNo` da ayrıca `android-invoice-lines:{snapshot.Id}` anahtarıyla önbelleğe alınıyor (`GetOrGroupInvoiceLinesAsync`); satırlar `JsonElement.Clone()` ile kopyalanıyor çünkü kaynak `JsonDocument`, isteği başlatan cevap gönderildikten sonra `dispose` edilir — clone, önbellek girdisinin o yaşam süresinden bağımsız kalmasını sağlıyor.

Testler: `Large_section_cache_is_shared_across_endpoints_that_request_it_together_or_alone` ve `Invoice_line_grouping_is_cached_once_and_reused_across_pages` — ikisi de önbellek girdisinin **referans eşitliği**yle (`BeSameAs`) yeniden hesaplanmadığını doğruluyor; düzeltme öncesi ikisi de başarısız olduğu doğrulandı.

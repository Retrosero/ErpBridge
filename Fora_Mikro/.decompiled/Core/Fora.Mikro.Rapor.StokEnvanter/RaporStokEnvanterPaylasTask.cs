...
public class RaporStokEnvanterPaylasTask
{
	public delegate void RaporStokEnvanterPaylasTaskBittiHandler(object sender, string Format, string cevap, MemoryStream SonucMS);

	private RaporStokEnvanterSonuc _rapor;
	...
	public event RaporStokEnvanterPaylasTaskBittiHandler OnRaporStokEnvanterPaylasTaskBitti;

	public void Baslat(string format, string dosyaismi, RaporStokEnvanterSonuc rapor)
	{
		_rapor = rapor;
		_format = format;
	...
	public void Main()
	{
		...
		try
		{
			...
			if (servis_kontrolu.Sonuc == enum_servis_kontrol_sonuc.ServisCalisiyor)
			{
				try
				{
					_SonucMS = RaporStokEnvanterRestClient.GetRaporStokEnvanterFile(servis_kontrolu.ServisAdresi, AppBase._FirmaID, AppBase._KullaniciAdi, AppBase._Sifre, _format, _dosyaismi, _rapor);
				}
				catch (Exception ex)
				{
		...
		this.OnRaporStokEnvanterPaylasTaskBitti(this, _format, _Cevap, _SonucMS);
	}
}

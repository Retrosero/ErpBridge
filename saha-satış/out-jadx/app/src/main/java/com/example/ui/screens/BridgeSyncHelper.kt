        ...
        try {
            ...
            while (hasMore) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val currentFingerprint = cariler.joinToString(",") { (it.actualCariKod ?: it.id ?: "").toString() }
                    if (cariler.isEmpty() || (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty())) {
                        if (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty()) log("Tekrarlayan sayfa algılandı, senkronizasyon tamamlandı.")
                        hasMore = false
                    } else {
                                        ...
                                        if (txRes.isSuccessful && txRes.body() != null) {
                                            ...
                                            if (items.isNotEmpty()) {
                                                ...
                                                for (item in items) {
                                                    ...
                                                    txList.add(
                                                        CustomerTx(
                                                            id = item.id ?: "TX-ERP-${(Math.random() * 100000).toInt()}",
                                                            date = formattedDate,
                                                            type = txType,
                ...
                for (mapped in allMappedCustomers) {
                    val existing = customerMap[mapped.id]
                    if (existing != null) {
                        val existingTxs = existing.transactions.filter { ext -> mapped.transactions.none { it.id == ext.id } }
                        mapped.transactions.addAll(existingTxs)
                    }
        ...
        try {
            ...
            var lastFingerprint = ""
                while (hasMoreBar) {
                    ...
                    if (barResponse.isSuccessful && barResponse.body() != null) {
                        ...
                        val items = body.items
                    val currentFingerprint = items.joinToString(",") { (it.barkod ?: "").toString() }
                    if (items.isEmpty() || (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty())) {
                        if (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty()) log("Tekrarlayan sayfa algılandı, senkronizasyon tamamlandı.")
                        hasMoreBar = false
                    } else {
            ...
            while (hasMore) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val currentFingerprint = urunler.joinToString(",") { (it.actualUrunKod ?: it.id ?: "").toString() }
                ...
                val currentList = AppDataStore.products.toList()
                    ...
                    if (items.isEmpty() || (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty())) {
                        ...
                        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
                            for (i in AppDataStore.customers.indices) {
                                ...
                                val custId = customer.id.trim()
                                val taxId = customer.taxNumber.trim()
                                val matches = items.filter { dto ->
                                    val dtoCari = dto.cariKod?.trim() ?: dto.cari_kod?.trim() ?: dto.cari_kodu?.trim()
                                    val idMatch = custId.isNotEmpty() && (dtoCari == custId || dto.erpRef?.trim() == custId || dto.erp?.trim() == custId || dto.customerCode?.trim() == custId || dto.erpKod?.trim() == custId)
                                    val taxMatch = taxId.isNotEmpty() && (dtoCari == taxId || dto.erpRef?.trim() == taxId || dto.erp?.trim() == taxId || dto.customerCode?.trim() == taxId || dto.erpKod?.trim() == taxId)
                                    idMatch || taxMatch
                                }
        ...
        try {
            ...
            while (hasMore) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val currentFingerprint = items.joinToString(",") { (it.evrakNo ?: "").toString() }
                    if (items.isEmpty() || (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty())) {
                        if (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty()) log("Tekrarlayan sayfa algılandı, senkronizasyon tamamlandı.")
                        hasMore = false
                        ...
                        db.withTransaction {
                            ...
                            for (fatura in items) {
                                ...
                                val custId = fatura.cariKod ?: ""
                                val matchedCustomer = AppDataStore.customers.find { 
                                    it.id.trim() == custId.trim() || it.taxNumber.trim() == custId.trim()
                                }
                                val custName = matchedCustomer?.name ?: "Müşteri ${fatura.cariKod}"
        ...
        try {
            ...
            while (hasMore) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    val body = response.body()!!
                    val items = body.items
                    val currentFingerprint = items.joinToString(",") { (it.kod ?: "").toString() }
                    if (items.isEmpty() || (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty())) {
                        if (currentFingerprint == lastFingerprint && lastFingerprint.isNotEmpty()) log("Tekrarlayan sayfa algılandı, senkronizasyon tamamlandı.")
                        hasMore = false
                    } else {

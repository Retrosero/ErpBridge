...
public static class AktarimBankaKriterParametre
{
	public static List<string> GetKriterAdlari(SqlBaglantiBilgileri BaglantiBilgileri, string DBName)
	{
		List<string> list = new List<string>();
		string commandText = "SELECT ParametreAltGrubu FROM _FORA_PARAMETRELER WITH (NOLOCK) WHERE ParametreProgram='BankaAktarim' AND ParametreAnaGrubu='Kriter' GROUP BY ParametreAltGrubu ORDER BY ParametreAltGrubu";
		try
		{
	...
	{
		string commandText = "DELETE _FORA_PARAMETRELER WHERE ParametreProgram='BankaAktarim' AND ParametreAnaGrubu='Kriter' AND ParametreAltGrubu=@ParametreAltGrubu";
		try
		{
			SqlDB sqlDB = new SqlDB();
			sqlDB.ConnectionOpen(BaglantiBilgileri, DBName);

...
public struct GenelEvrakData
{
	public static GenelEvrak ConvertEvrak(SqlConnection openedconnection, Evrak evrak, enum_satir_gruplandirma_secenekleri gruplandirma_secenekleri)
	{
		...
		genelEvrak.temsilci = CariPersonelData.GetCariPersonel(openedconnection, evrak.TemsilciKodu);
	...
	private static List<GenelEvrakSatirlari> ConvertSiparisSatirlar(SqlConnection openedconnection, Evrak evrak)
	{
		List<GenelEvrakSatirlari> list = new List<GenelEvrakSatirlari>();
		foreach (SIPARISLER item in evrak.GetSiparisler())
		{
			...
			Stok stok = StokData.GetStok(openedconnection, item.sip_stok_kod, enum_toptan_perakende.Toptan);
			...
			genelEvrakSatirlari.cari_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.sip_cari_sormerk);
			genelEvrakSatirlari.stok_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.sip_stok_sormerk);
			genelEvrakSatirlari.karsi_sorumluluk_merkezi = new SorumlulukMerkezi();
			genelEvrakSatirlari.proje = ProjeData.GetProje(openedconnection, item.sip_projekodu);
			...
			genelEvrakSatirlari.altgrup_adi = AltGrupData.GetAltGrupAdi(openedconnection, genelEvrakSatirlari.altgrup_kod, stok.sto_anagrup_kod);
			genelEvrakSatirlari.anagrup_kod = stok.sto_anagrup_kod;
			genelEvrakSatirlari.anagrup_adi = AnaGrupData.GetAnaGrupAdi(openedconnection, genelEvrakSatirlari.anagrup_kod);
			genelEvrakSatirlari.sektor_kodu = stok.sto_sektor_kodu;
			genelEvrakSatirlari.sektor_adi = StokSektorData.GetSektorAdi(openedconnection, genelEvrakSatirlari.sektor_kodu);
			genelEvrakSatirlari.marka_kodu = stok.sto_marka_kodu;
			genelEvrakSatirlari.marka_adi = MarkaData.GetMarkaAdi(openedconnection, genelEvrakSatirlari.marka_kodu);
			genelEvrakSatirlari.model_kodu = stok.sto_model_kodu;
			genelEvrakSatirlari.model_adi = StokModelData.GetModelAdi(openedconnection, genelEvrakSatirlari.model_kodu);
			genelEvrakSatirlari.uretici_kodu = stok.sto_uretici_kodu;
			genelEvrakSatirlari.uretici_adi = UreticiData.GetUreticiAdi(openedconnection, genelEvrakSatirlari.uretici_kodu);
			genelEvrakSatirlari.reyon_kodu = stok.sto_reyon_kodu;
			genelEvrakSatirlari.reyon_adi = ReyonData.GetReyonAdi(openedconnection, genelEvrakSatirlari.uretici_kodu);
			...
			try
			{
				foreach (string item2 in StokData.GetStokBarkodlari(openedconnection, item.sip_stok_kod))
				{
	...
	private static List<GenelEvrakSatirlari> ConvertDepolarArasiSiparisSatirlar(SqlConnection openedconnection, Evrak evrak)
	{
		List<GenelEvrakSatirlari> list = new List<GenelEvrakSatirlari>();
		foreach (DEPOLAR_ARASI_SIPARISLER item in evrak.GetDepolarArasiSiparisHareketleri())
		{
			GenelEvrakSatirlari genelEvrakSatirlari = new GenelEvrakSatirlari();
			genelEvrakSatirlari.satir_cinsi = GenelEvrakSatirCinsleri.Stok;
			Stok stok = StokData.GetStok(openedconnection, item.ssip_stok_kod, enum_toptan_perakende.Toptan);
			...
			genelEvrakSatirlari.miktar2 = 0.0;
			genelEvrakSatirlari.cari_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.ssip_sormerkezi);
			genelEvrakSatirlari.stok_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.ssip_sormerkezi);
			genelEvrakSatirlari.karsi_sorumluluk_merkezi = new SorumlulukMerkezi();
			genelEvrakSatirlari.proje = ProjeData.GetProje(openedconnection, item.ssip_projekodu);
			...
			try
			{
				foreach (string item2 in StokData.GetStokBarkodlari(openedconnection, item.ssip_stok_kod))
				{
					string[] array = item2.Split('|');
					switch (array[0])
					{
	...
	private static List<GenelEvrakSatirlari> ConvertFaturaSatirlar(SqlConnection openedconnection, Evrak evrak)
	{
		List<GenelEvrakSatirlari> list = new List<GenelEvrakSatirlari>();
		foreach (STOK_HAREKETLERI item in evrak.GetStokHareketleri())
		{
			GenelEvrakSatirlari genelEvrakSatirlari = new GenelEvrakSatirlari();
			genelEvrakSatirlari.satir_cinsi = GenelEvrakSatirCinsleri.Stok;
			Stok stok = StokData.GetStok(openedconnection, item.sth_stok_kod, enum_toptan_perakende.Toptan);
			...
			genelEvrakSatirlari.cari_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.sth_cari_srm_merkezi);
			genelEvrakSatirlari.stok_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.sth_stok_srm_merkezi);
			genelEvrakSatirlari.karsi_sorumluluk_merkezi = new SorumlulukMerkezi();
			genelEvrakSatirlari.proje = ProjeData.GetProje(openedconnection, item.sth_proje_kodu);
			...
			try
			{
				foreach (string item2 in StokData.GetStokBarkodlari(openedconnection, item.sth_stok_kod))
				{
					string[] array = item2.Split('|');
					switch (array[0])
					{
		...
		foreach (CARI_HESAP_HAREKETLERI item3 in evrak.GetHizmetHareketleri())
		{
			...
			Hizmet hizmet = HizmetData.GetHizmet(openedconnection, item3.cha_kasa_hizkod);
			...
			genelEvrakSatirlari2.cari_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item3.cha_srmrkkodu);
			genelEvrakSatirlari2.stok_sorumluluk_merkezi = new SorumlulukMerkezi();
			genelEvrakSatirlari2.karsi_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item3.cha_karsisrmrkkodu);
			genelEvrakSatirlari2.proje = ProjeData.GetProje(openedconnection, item3.cha_projekodu);
	...
	private static List<GenelEvrakSatirlari> ConvertTahsilatSatirlar(SqlConnection openedconnection, Evrak evrak)
	{
		List<GenelEvrakSatirlari> list = new List<GenelEvrakSatirlari>();
		foreach (CARI_HESAP_HAREKETLERI item in evrak.GetTahsilatHareketleri())
		{
			...
			genelEvrakSatirlari.cari_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.cha_srmrkkodu);
			genelEvrakSatirlari.stok_sorumluluk_merkezi = new SorumlulukMerkezi();
			genelEvrakSatirlari.karsi_sorumluluk_merkezi = SorumlulukMerkeziData.GetSorumlulukMerkezi(openedconnection, item.cha_karsisrmrkkodu);
			genelEvrakSatirlari.proje = ProjeData.GetProje(openedconnection, item.cha_projekodu);

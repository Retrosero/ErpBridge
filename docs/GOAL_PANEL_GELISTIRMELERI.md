...
| # | Karar | Gerekçe |
|---|---|---|
| D1 | Eski telefon sürümü DEPO rolünü tanımadığı için tüm ekranları açar → yalnız DEPO rolü olan kullanıcıya telefonda giriş **yalnız `versionCode ≥ Mobile:MinWarehousePhoneVersionCode`** ise verilir; ayar boşken eski davranış (`ROLE_NOT_ALLOWED_ON_PHONE`) sürer | Depocunun eski sürümden satış girmesi engellenir |
...
9. `gh pr merge <n> --squash --delete-branch` ("main is already used by worktree" hatası merge'u bozmaz —
   `gh pr view <n> --json state` ile doğrula).
10. `GOAL_PANEL_DURUM.md` **aynı PR içinde** güncellenir (ayrı belge PR'ı açılmaz). Bilgi bankasını eskiten
    değişiklik aynı PR'da KB'ye işlenir.

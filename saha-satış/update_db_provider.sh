sed -i 's/\.fallbackToDestructiveMigration()//g' app/src/main/java/com/example/data/database/DatabaseProvider.kt
sed -i 's/\.addMigrations(MIGRATION_8_9, MIGRATION_9_10)/\.addMigrations(MIGRATION_8_9, MIGRATION_9_10, MIGRATION_10_11)/g' app/src/main/java/com/example/data/database/DatabaseProvider.kt

cat << 'INNEREOF' > patch_provider.py
import re

...
- `agent_heartbeat_log` *(Log Merkezi L3f)*: ajan heartbeat geçmişi — durum/sürüm/senkron sonucu/hata değişince veya 15 dakikada bir satır. `AgentId`, `TenantId`, `RecordedAtUtc/Ms`, `Status(32)`, `QueueDepth`, `AppVersion(64)`, `HostKind(16)` service|ui, `LastSyncAtUtc`, `LastSyncResult(32)` OK|FAILED, `LastError(1000)` maskeli. Index `(AgentId, RecordedAtMs)`, `(RecordedAtMs)`. `LogRetention` WARN süresiyle siler.
- `agents` ek kolonlar *(Log Merkezi L3f, nullable)*: `AppVersion`, `HostKind`, `LastSyncAtUtc`, `LastSyncResult`, `LastError` (maskeli; önceden heartbeat'in gönderdiği hata atılıyordu), `LastHeartbeatLoggedAtUtc`.
- `jobs.CorrelationId` *(Log Merkezi L3g, nullable, 128, index)*: işi oluşturan isteğin iz kimliği.

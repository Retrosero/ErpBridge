...
public static class TahsilatYazdirmaSqlite
{
	public static List<string> TextOlustur(SqliteConnection OpenedConnection, Evrak evrak, YaziciAyarlari yaziciayarlari)
	{
		...
		foreach (Parametreler item in yaziciayarlari.alanlar)
		{
			if (item._GetParametre("BasilacakAlan")._GetInt == 0)
			{
				...
				{
					text = StatikAlanVeriGetir(OpenedConnection, evrak, getString);
				}
		...
		foreach (Parametreler item4 in yaziciayarlari.alanlar)
		{
			if (item4._GetParametre("BasilacakAlan")._GetInt == 2)
			{
				string getString3 = item4._GetParametre("VeriTipi")._GetString;
				string text3 = item4._GetParametre("Veri")._GetString;
				int getInt13 = item4._GetParametre("Kolon")._GetInt;
				int getInt14 = item4._GetParametre("Genislik")._GetInt;
				enum_Yazdirma_Hizalama getInt15 = (enum_Yazdirma_Hizalama)item4._GetParametre("Hizalama")._GetInt;
				bool getBoolean = item4._GetParametre("DetayinBittigiYereKaydir")._GetBoolean;
				if (getString3 != "[statik_metin]")
				{
					text3 = StatikAlanVeriGetir(OpenedConnection, evrak, getString3);
				}
				...
				{
				case enum_Yazdirma_Hizalama.Orta:
					num7 += (getInt14 - text3.Length) / 2;
					break;
				case enum_Yazdirma_Hizalama.Sag:
					num7 += getInt14 - text3.Length;
					break;
				}
				int getInt16 = item4._GetParametre("Satir")._GetInt;
				getInt16 = ((!getBoolean) ? (getInt16 + (getInt3 + getInt5 * getInt4)) : (getInt16 + (num2 - 1)));
				list[getInt16 - 1] = list[getInt16 - 1].Remove(num7, text3.Length);
				list[getInt16 - 1] = list[getInt16 - 1].Insert(num7, text3);
			}
		}
		for (int j = 0; j < list.Count; j++)
		{
			list[j] = list[j].TrimEnd(Array.Empty<char>());
		}
		int num8 = getInt2;
		while (num8 > 0 && list[num8 - 1] == "")
		{
	...
	private static string StatikAlanVeriGetir(SqliteConnection OpenedConnection, Evrak evrak, string veriadi)
	{
		switch (veriadi)
		{
		case "[cari_kod]":
		...
		case "[temsilci_adi]":
			return TemsilciSqlite.GetTemsilciAdi(OpenedConnection, evrak.TemsilciKodu);
		case "[ara_toplam]":
	...
	private static string doubletostring(double value)
	{
		...
		return value.ToString("N", cultureInfo);
	}
